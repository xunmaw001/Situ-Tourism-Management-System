            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">娱乐项目收藏展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">娱乐项目名称</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="yulexiangmuName" id="yulexiangmuName" placeholder="娱乐项目名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../yulexiangmu/detail.jsp?id='+item.yulexiangmuId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.yulexiangmuPhoto?item.yulexiangmuPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../yulexiangmu/detail.jsp?id='+item.yulexiangmuId)"><span>{{item.yulexiangmuName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.yulexiangmuNewMoney"><span style="color: red;">{{item.yulexiangmuNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteYulexiangmuCollection(item.id)">删除 {{item.yulexiangmuName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            yulexiangmuCollectionTypes: 1,//类型
            limit: 10,
        ...
        filters: {
            //把 yulexiangmuCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            yulexiangmuCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteYulexiangmuCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('yulexiangmuCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#yulexiangmuName').val() != null && jquery('#yulexiangmuName').val() != "" && jquery('#yulexiangmuName').val() != "null" ) {
                param['yulexiangmuName'] = jquery('#yulexiangmuName').val();
            }
                                                                                
            // 当前表
                                                    // 获取列表数据
            http.request('yulexiangmuCollection/list', 'get', param, function(res) {
