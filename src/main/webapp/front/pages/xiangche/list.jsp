            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">相册展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="xiangcheName" id="xiangcheName" placeholder="相册名" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="xiangcheTypes" id="xiangcheTypes" lay-filter="xiangcheTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in xiangcheTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../xiangche/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.xiangchePhoto?item.xiangchePhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../xiangche/detail.jsp?id='+item.id)"><span>{{item.xiangcheName}}</span></div>
                                <div class="ng-prslist-tit" v-if="item.xiangcheNewMoney"><span style="color: red;">{{item.xiangcheNewMoney}} RMB</span></div>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 xiangcheContent 字段去除无用字符 如果字段长度大于60  就截取到60
            xiangcheContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#xiangcheName').val() != null && jquery('#xiangcheName').val() != "" && jquery('#xiangcheName').val() != "null" ) {
                param['xiangcheName'] = jquery('#xiangcheName').val();
            }
         
            var xiangcheTypes = $("#xiangcheTypes").find("option:selected").val();
            if (xiangcheTypes != null && xiangcheTypes != "" && xiangcheTypes != "null") {
                param['xiangcheTypes'] = xiangcheTypes;
            }
                                            // 获取列表数据
    ...
    // 当前表的字典表
       ...
       function xiangcheTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xiangche_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.xiangcheTypesList = res.data.list;
               }
           });
       }
